...
- **New subs (7d)** = `start_date` in `[today-7, today-1]`. **Cancellations (7d)** = distinct users with `end_date` in that window. **newSubsToday** = `start_date` ≥ midnight today. **Both EXCLUDE `upgrade …` terms** — those are existing subscribers migrated to new rate cards by the monthly price-upgrade automation (Piano writes a fresh subscription record dated today), not genuine new acquisition. Winbacks + first-time subs are counted.
